...
<script>
  function logOut(){
    var destroy = <?php session_unset();?>
    window.location.href = "http://localhost:8888/CS135---Project-master/index.php";
  }
</script>
...
function isInDB($con, $roomName) {
    ...
    if ($roomName != null) {
      $getRoom = $con->prepare("SELECT room_id FROM room WHERE roomname = ?");
...
function getRoomInfo($con, $roomInfo_array, $roomName) {
  ...
  if ($roomName != null ) {
    $getInfo = $con->prepare("SELECT room_id, roomname, capacity, location, computer, blackboard FROM room WHERE roomname = ?");
  ...
  $roomInfo_array["roomname"] = $thisRoomName; 
	...
	<header>
      ...
      <div class="container-fluid">
        ...
        <ul class="nav navbar-nav">
          ...
          <li class = "active"><a href="AdminUpdateRoomFeaturesv2.php">Room Updates</a></li>
        </ul>
...
<div class="panel panel-primary">
  ...
  <div class="panel-body"><?php if(isset($roomInfo_array["roomname"])) {
    echo $roomInfo_array["roomname"];
  } ?></div>
</div>
</center>
...
<input hidden name="RoomName" value ="<?php if(isset($roomInfo_array["roomname"])) {echo $roomInfo_array["roomname"];} ?>"> </input>
...
if (!isset($roomName)) {
  global $roomName;
  if(isset($roomInfo_array["roomname"])){
   $roomName = $roomInfo_array["roomname"];
  }
}
...
function updateCapacity ($con, $roomName, $oldCapacity, $newCapacity) {
  ...
  if ($newCapacity <= 0) {
    ...
    if ($oldCapacity != $newCapacity) {
      $updateRoom = $con->prepare("UPDATE room SET capacity = ? WHERE roomname = ?");
...
function updateComputer ($con, $roomName, $oldComputer, $newComputer) {
  if ($newComputer != $oldComputer) {
    $updateRoom = $con->prepare("UPDATE room SET computer = ? WHERE roomname = ?");
...
function updateWhiteboard($con, $roomName, $oldWhiteboard, $newWhiteboard) {
   if($oldWhiteboard != $newWhiteboard) {
     $updateRoom = $con->prepare("UPDATE room SET blackboard = ? WHERE roomname = ?");
